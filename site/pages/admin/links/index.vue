...
export default {
  ...
  methods: {
    ...
    addSubmit() {
      const me = this
      this.$axios
        .post('/api/admin/links', this.addForm)
        .then((data) => {
    ...
    async detect() {
      ...
      try {
        ...
        if (flag) {
          const data = await this.$axios.post('/api/admin/link/detect', {
    ...
    editSubmit() {
      const me = this
      this.$axios
        .put('/api/admin/links/' + me.editForm.id, me.editForm)
        .then((data) => {
    ...
    async deleteSubmit(row) {
      const me = this
      await this.$confirm('是否确认删除该链接吗?', '提示', {
        ...
        .then((confirm) => {
          try {
            this.$axios.delete('/api/admin/links/' + row.id)
